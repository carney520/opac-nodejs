...
exports.destroy = function(req,res,next){
  ...
  req.filter.require('user_id')
    .done(function(err,params){
      ...
      //确保删除的楼层是用户自己的
      BookComment.findOneAndUpdate({book_id:bookid,comment_id:comment_id,'user.id':req.user.card_no},
          {deleted:true,
            $set:{user:null,reply_user:null,content:null,reply_id:null,ups:[]}})
      .then(function(product){
        if(product){
...
exports.undo_like = function(req,res,next){
  ...
  BookComment.findOneAndUpdate({book_id:bookid,comment_id:comment_id},
      {$pull:{ups:user_id}})
  .then(function(comment){
    if(comment){
      res.json({
