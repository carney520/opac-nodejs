...
exports.update = function(req, res, next) {
  ...
  req.filter
    .permit('email', 'tel', 'mobile', 'signature', 'introduce')
...
exports.change_password = function(req, res, next) {
  ...
  req.filter
    .require('old_password', 'new_password', 'verify_password')
    .done(function(err, params) {
      if (params.new_password !== params.verify_password) {
        res.format({
          json: function() {
            res
              .status(400)
              .json({ code: 400, message: 'password inconsistent' })
          },
          html: function() {
            ...
            if (!product.validPassword(params.old_password)) {
              res.format({
                json: function() {
                  res
                    .status(400)
                    .json({ code: 400, message: 'invaild password' })
                },
                html: function() {
          ...
          .then(function(records) {
            ...
            if (records) {
              ...
              records.forEach(function(record) {
                ...
                _.extend(
                  result,
                  _.omit(record.toObject(), '_id', '__v', 'penalty')
                )
    ...
    .then(function(results) {
      if (results.record) {
        ...
        if (due_date.getTime() <= Date.now()) {
          ...
          res
            .status(403)
            .json({ code: 403, message: 'renewal time limit reached' })
        } else {
          record.renewals_times += 1
          record.due_date = due_date.setTime(
            due_date.getTime() + max_days_renewals * one_day
          )
          return record.save()
        }
      } else {
    ...
    .then(function(record) {
      if (record) {
        res.json({
          code: 200,
          message: 'success',
          result: _.omit(record.toObject(), '_id', 'penalty', '__v'),
        })
      }
    })
    .catch(function(err) {
