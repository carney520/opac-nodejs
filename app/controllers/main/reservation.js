    ...
    Promise.props({
      ...
      .then(function(results) {
        if (results) {
          ...
          if (
            !_.isEmpty(queue) &&
            _.find(queue, function(v) {
              return v.user_id === user.card_no
            })
          ) {
            return res
              .status(409)
              .json({ code: 409, message: 'Alreay reservated' })
          }
            ...
            if (reservation.book_available) {
              ...
              if (book_availables > 0) {
                ...
                reservation.latest_loan_date.setTime(
                  now + type.reservation_expire * one_day
                )
                //TODO 通知预约读者
          ...
          if (book_availables > 0) {
            ...
            newReservation.latest_loan_date.setTime(
              now + type.reservation_expire * one_day
            )
            book_availables -= 1
            //TODO 通知预约者
...
exports.show = function(req, res, next) {
  ...
  req.filter.permit('user_id', 'book_id').done(function(err, params) {
    if (err) return next(err)
    ...
    if (userid && bookid) {
      ...
      promise = BookReservation.find({ user_id: userid }).sort(
        'reservation_date'
      )
    }
