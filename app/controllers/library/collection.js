...
exports.create = function(req, res, next) {
  ...
  collection_params(req, function(err, params) {
    ...
    Book.findByIdAndUpdate(bookId, {
      $push: { books: params },
      $inc: { holdings: 1, availables: availables },
    })
...
exports.update = function(req, res, next) {
  ...
  collection_params(req, function(err, params) {
    ...
    Book.findOne({ _id: bookId, 'books.accession_no': collection_id })
      ...
      .then(function(product) {
        if (product) {
          var collection = _.find(product.books, function(value) {
            return value.accession_no === collection_id
          })
          //状态转变
...
exports.destroy = function(req, res, next) {
  var bookId = req.params.id,
    collection_id = req.params.collection_id
  Book.findByIdAndUpdate(bookId, {
    $pull: { books: { accession_no: collection_id } },
    $inc: { holdings: -1, availables: -1 },
  })
    .then(function(product) {
      if (product) {
...
var collection_params = function(req, callback) {
  req.filter
    .require('collection_site', 'status')
    .permit('accession_no')
    .done(callback)
}
