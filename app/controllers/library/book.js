    ...
    .done(function(err, params) {
      ...
      search(params)
        .then(function(results) {
          ...
          console.log(results)
          //用于设置请求字符串
          res.locals.setSearchString = function(
            field,
            value,
            more_key_and_value
          ) {
            var key_values = _.partition(_.toArray(arguments), function(n, i) {
              return i % 2 === 0
            }),
          ...
          //分页
          res.locals.pagination = new Pagination(
            results.total,
            results.start,
            results.expect_count
          )
...
exports.show = function(req, res, next) {
  ...
  Promise.props({
    ...
    types: BookType.all(),
    reservations: BookReservation.find({ book_id: id }).sort({
      reservation_date: 1,
    }),
  })
    .then(function(product) {
      if (product.book) {
        ...
        product.reservable = book.availables - reservations.length >
          type.reservation_threshold
          ? false
          : true
...
var search = function(doc) {
  ...
  var props = {
    ...
    /*缩检*/
    //结果集包含的标签
    tags: Book.distinct('tags', querydoc),
    //分类
    categories: Book.aggregate()
      .match(querydoc)
      .project({ second_category: 1 })
      .group({ _id: '$second_category', count: { $sum: 1 } })
      .exec(),
    //图书类型
    types: Book.aggregate()
      .match(querydoc)
      .project({ type: 1 })
      .group({ _id: '$type', count: { $sum: 1 } })
      .exec(),
    //出版年
    years: Book.aggregate()
      .match(querydoc)
  ...
  return new Promise.props(props).then(function(results) {
