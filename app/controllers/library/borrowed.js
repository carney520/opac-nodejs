    ...
    Reader.findOne({ card_no: card_no })
      .then(function(product) {
        if (product) {
          ...
          res
            .status(404)
            .json({ code: 404, message: 'Reader not found', err: 'reader' })
        }
      })
      .then(function(result) {
        if (result) {
          ...
          if (promises.length > 0) {
            ...
            //没有超期书籍，分析可借空间
            var count = _.countBy(records, function(value) {
              return value.type
            })
            ...
            return Book.findOne({
              _id: book_id,
              'books.accession_no': accession_no,
            })
          }
        }
      })
      .then(function(results) {
        if (results) {
          if (_.isArray(results)) {
            ...
            var book = results,
              book_type = book.type,
              collection = _.find(book.books, function(value) {
                return value.accession_no === accession_no
              })
        ...
        if (book) {
          ...
          new BookBorrowed({
            ...
            .catch(function(err) {
              ...
              var collection = _.find(book.books, function(value) {
                return value.accession_no === accession_no
              })
...
exports.destroy = function(req, res, next) {
  ...
  req.filter.require('book_id').permit('amount').done(function(err, params) {
    if (err) return next(err)
      ...
      .then(function(results) {
        ...
        if (record) {
          ...
          if (reservation) {
            ...
            reservation.latest_loan_date.setTime(
              Date.now() + type.reservation_expire * one_day
            )
            console.log(
              'notice ' +
...
var borrowed_params = function(req, callback) {
  req.filter.require('accession_no', 'card_no', 'book_id').done(callback)
}
