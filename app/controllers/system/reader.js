...
exports.index=function(req,res,next){
  Reader.find()
    .sort({created_at:-1})
    .select({name:1,card_no:1,avatar:1,created_at:1,status:1,notes:1})
    .exec()
...
exports.destroy = function(req,res,next){
  ...
  Reader.findOneAndRemove({card_no:card_no})
    .then(function(reader){
      if(reader)
        res.status(200).json({code:200,message:'deleted',card_no:card_no,name:reader.name});
      else{
        next(); //not found
      }
    }).catch(function(err){
...
var reader_params = function(req,cb){
  req.filter.require('name','card_no').permit('email','tel','mobile','notes','password').done(cb);
};
