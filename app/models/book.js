...
var incr = function(key, dbkey, threshold) {
  ...
  return function(bookid, incrValue, initialize) {
    ...
    initialize =
      initialize ||
      Model.findById(bookid, { statistics_info: 1 }).then(function(book) {
        if (book) {
          return book.statistics_info[dbkey]
        }
      })

    cacheable.incrWriteBack
      .call(
        this,
        bookid,
        key,
...
;['viewed', 'marked', 'replied'].forEach(function(key) {
  bookSchema.statics[
    'get' +
      key.replace(/^./, function(match) {
        return match.toUpperCase()
      })
  ] = function(id) {
    return cacheable.getIncr.call(this, id, key).then(function(value) {
