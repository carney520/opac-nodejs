  ...
  return new Promise(function(resolve,reject){
    ...
    cache.isExist(keyName,function(err,result){
      ...
      if(result === 1){
        //键存在
        //检查是否老于数据库
        cache.client.mget(lastUpdateTimeStamp,lastPullOutTimeStamp)
        ...
        .then(function(results){
          ...
          if(result === 1 && lastPullOut >= lastUpdate){
            cache.hvals(keyName,function(err,values){
              if(err) return reject(err);
              if(values){
                values = _.map(values,function(value){return new Model(value);});
              }
              resolve(values);
            });
          }else{
              ...
              .then(function(values){
                if(values){
                  ...
                  values.forEach(function(value){
                    pipeline.hset(keyName,value[keyfield],JSON.stringify(value));
                  });
