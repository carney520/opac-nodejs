...
var url_for = function(path_alias, options) {
  ...
  if (options && options.only_path !== undefined) only_path = options.only_path
  else if (
    options &&
    (options.query ||
      options.host ||
    ...
    if (ids.length === resource.required_params) {
      ...
      for (i = 0; i < ids.length; i++) {
        absolute_path = absolute_path.replace(
          resource.router_variables[i],
          ids[i].toString()
        )
      }
      /*构造url */
      if (only_path) return absolute_path
      else {
      ...
      throw new Error(
        'Required parameter missing: expect ' +
          resource.router_variables.join(', ')
      )
    }
...
var RouterAgent = function(
  router,
  prefix,
  resource_name,
  no_append_resource_name
) {
  this._router = router
  this._prefix = prefix
...
var includesPatterns = function(path) {
  ...
  for (var i = 0; i < patterns.length; i++) {
    if (path.includes(patterns[i])) return true
  }
  return false
}
...
RouterAgent.prototype.create_alias = function(path) {
  var variables = _.reject(path.split('/'), function(value) {
    return value === ''
  }),
    alias = [variables[0]],
    prev
...
RouterAgent.prototype._compile_path = function(path, as) {
  ...
  if (_.isRegExp(path) || includesPatterns(path)) return false
  //路由别名
...
methods.concat(['del', 'all']).forEach(function(method) {
  RouterAgent.prototype[method] = function(path, as) {
    var customize_alias
    if (!_.isFunction(as)) customize_alias = as
