  ...
  try {
    if (!_.isEmpty(requires)) {
      for (var i = 0; i < requires.length; i++) {
        ...
        if (this._params[value]) this.parameters[value] = this._params[value]
        else throw new ParameterMissingError(value, message)
      }
    }
...
var hash_filter = function(hash, source, target) {
  _.forEach(hash, function(value, key) {
    ...
    if (_.isEmpty(value)) {
      if (
        (_.isObject(value) && _.isObject(source[key])) ||
