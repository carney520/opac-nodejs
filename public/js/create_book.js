$(document).ready(function() {
  ...
  var itemClickHandler = function(event) {
    ...
    //封面
    var $cover = $('<img>').attr('src', bookData.image).addClass('book-cover')
    var $coverUrl = $('input[name="cover_url"]').val(bookData.image)
    ...
    return $.ajax('http://api.douban.com/v2/book/search', {
      ...
      .done(function(results) {
        ...
        items.forEach(function(item) {
          ...
          //封面
          var $cover = $('<img src="' + item.image + '"></img>').addClass(
            'item-cover'
          )
          //标题
          var $header = $('<h4>' + item.title + '</h4>').addClass(
  ...
  $('#main-form').submit(function(event) {
    ...
    if (!/^\d{4}[-.]\d{1,2}[-.]\d{1,2}/.test($publicationYear.val())) {
      $publicationYear
        ...
        .addClass('has-error')
        .append(
          $('<span id="year_help">').addClass('help-block').html('日期格式有误')
        )
      return false
    }
    var data = {}
