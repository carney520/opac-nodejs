$(document).ready(function() {
      ...
      $.ajax({
        ...
        .done(function(data) {
          var tags = data.tags || [],
            ...
            recommendTags = $('meta[name="book-tags"]')
              .attr('content')
              .split(' '),
          ...
          var $window = showWindow($tagContent, function() {
            ...
            tags = _.map(tags.split(' '), function(value) {
              return value.trim()
            })
            $.ajax({
  ...
  if (!userid) {
    //未登录
    $addComment.addClass('disabled').html('未登录').click(function() {
      return loginFirst()
    })
  ...
  $('.star-inner').hover(hover, hover).click(function() {
    var $this = $(this),
      $parent = $this.parent('.star'),
      $score = $('.star-score'),
      value = $this.data('value')
  ...
  var setStar = function(value, relative_value) {
    ...
    $star
      .removeClass('star-' + relative_value)
      .addClass('star-' + value)
      .data('value', value)
    $('input[name="score"]').val(value)
    $score.html(value)
  ...
  var newCommentItem = function(data) {
    ...
    if (data.deleted) {
      commentItem = $('<div class="comment-item-deleted">').html(
        data.comment_id + ' 楼已删除'
      )
    } else {
      commentItem = Template.comment_item({
        // jade 客户端模板
        ...
        url_for_reply_user:
          data.reply_user.id &&
            userTemplateforTag.urlFor('user', data.reply_user.id), //被回复者的主页
  ...
  $commentList.on('click', '.comment-delete', function(event) {
    ...
    $.ajax({
      ...
      .done(function(data) {
        var commentItem = $('<div class="comment-item-deleted">').html(
          comment_id + ' 楼已删除'
        )
        $parent.slideUp()
  ...
  $commentList.on('click', '.comment-reply', function(event) {
    ...
    $commentDescription
      .html('回复')
      .append(
        $('<a class="id">').attr('href', '#' + comment_id).html(comment_id)
      )
      .append(
        $('<a class="user">')
          .attr('href', userTemplateforTag.urlFor('user', userid))
          .html(username)
      )
    //隐藏评级
  ...
  $commentList.on('click', '.comment-agree', function(event) {
    ...
    if (!userid) return loginFirst()

    if (liked) {
      //取消赞
      $.ajax({
        url: userTemplateforTag.urlFor(
          'book_comment_like',
          bookid,
          comment_id,
          userid
  ...
  $('.lent_out').click(function(event) {
    ...
    if (method === 'lent_out') {
      //借出
      showWindow(
        $(Template.card_no_inputer({ title: '请输入读者证件号', name: 'card_no' })),
        function() {
