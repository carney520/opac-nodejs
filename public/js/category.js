$(document).ready(function(){
  ...
  var td = function(inner){
    if(inner)
      return $td.clone().append(inner.clone());
    else
      return $td.clone();
  };
  ...
  $('#add-category').click(function(event){
    ...
    if($input.hasClass('add-input-hidden')){
      ...
      .done(function(data){
        ...
        //添加进列表
        var $item = $('<tr>').addClass('category-item')
          .data('id',data.id)
        ...
        $item.append(td().html(data.name).addClass('category-name'))
  ...
  function categoryMethod(event){
    ...
    if(method === 'update'){
      ...
      showModal('警告','确认删除: <strong>' + categoryName +'</strong>?',function(){
      ...
      showModal('输入子分类名称',modalInput(),function(){
        ...
        .done(function(data){
          ...
          //添加进列表
          $('<span>'+data.name+'</span>').addClass('category-child')
      ...
      showModal('警告','确认删除子分类: <strong>'+ childName +'</strong>',function(){
        $button.html(waiting());
        $.ajax({
